...
public class Dealer implements Runnable {
    ...
    private Integer[] set;  
    public volatile boolean waitForTheDealerToReshuffle;
    ...
    public int ThePlayerWhoAnnouncedFirst()
    {
    ...
    {      
        ...
        while(!players[id].wait & !terminate);

        if (!areTheCardsAvailable(players[id])) 
        {
            removeTokensFromUnavailableCards(players[id]);
            players[id].setState(State.Continue);
        }
        else if (players[id].getTokensToSlots().size() == env.config.featureSize)
        {
    ...
    public void removeTokensFromUnavailableCards(Player player)
    {
        Integer slot;
        for (int i = 0; i < player.getTokensToSlots().size(); i++)
        {
            slot = player.getTokensToSlots().get(i);
            if (table.slotToCard[slot] == null)          
            {
                player.getTokensToSlots().remove(slot);
    ...
    public void lockTheCards()
    {
        for (int i = 0; i < set.length; i++)
        {
            table.slotToCard[set[i]] = null;
        }
    }
